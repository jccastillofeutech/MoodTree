        ...
        if (user) {
            ...
            getDoc(userDocRef).then(docSnap => {
                if (docSnap.exists()) {
                    ...
                    updateUsernameDisplays(username);
                }
            });
        } else {
            username = null;
...
function setupLogMoodPage() {
    ...
    createSeedButton.addEventListener('click', () => {
        ...
        const moodData = {
            ...
            sadness: sliders.sadness.value,
            anger: sliders.anger.value,
            calmness: sliders.calmness.value,
            work: sliders.work.value,
            sleep: sliders.sleep.value,
            moodColor: moodColor,
            moodName: moodName,
...
function setupSimulatorPage() {
                ...
                if (docSnap.exists()) {
                    ...
                    if (dailyMood && dailyMood.date !== todayKey) {
                        ...
                        updateDoc(userDocRef, {
                            dailyMood: deleteField(),
                            treeState: deleteField(),
                            treeClicks: deleteField()
                        });
                        state = 'unplanted';
                        clicks = 0;
                    } else {
                        state = data.treeState || 'unplanted';
                        clicks = data.treeClicks || 0;
                    }
                } else {
                    state = 'unplanted';
                    clicks = 0;
    ...
    function initializeSimulator() {
        ...
        function updateUI() {
            ...
            if (user) {
                const userDocRef = doc(db, 'users', user.uid);
                setDoc(userDocRef, { treeState: state, treeClicks: clicks }, { merge: true });
            }
        }
        ...
        shovelBtn.addEventListener('click', () => {
            if (state === 'mature') {
                ...
                const userDocRef = doc(db, 'users', user.uid);
                getDoc(userDocRef).then(docSnap => {
                    const history = docSnap.exists() ? docSnap.data().moodHistory || {} : {};
...
function setupCalendarPage() {
    ...
    onAuthStateChanged(auth, user => {
        if (user) {
            ...
            getDoc(userDocRef).then(docSnap => {
                if (docSnap.exists()) {
                    const history = docSnap.data().moodHistory || {};
                    renderCalendar(currentDate.getFullYear(), currentDate.getMonth(), history);

                    if (showDialogOnLoad && dialogDateKey) {
                        const dateParts = dialogDateKey.split('-').map(Number);
                        if (dateParts[0] === currentDate.getFullYear() && (dateParts[1] - 1) === currentDate.getMonth()) {
                            const dayElement = calendarGrid.querySelector(`[data-date-key="${dialogDateKey}"]`);
                            if (dayElement) {
                                showShovelDialog(dialogDateKey, history);
    ...
    function showShovelDialog(dateKey, history) {
    ...
    function showCalendarDayDetailsDialog(dateKey, history) {
        const entry = history[dateKey];
        if (!entry) return;
    ...
    function renderCalendar(year, month, history) {
        ...
        monthTitle.textContent = date.toLocaleString('en-US', { month: 'long', year: 'numeric' });
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (history[dateKey]) {
                ...
                daySquare.addEventListener('click', () => showCalendarDayDetailsDialog(dateKey, history));
            }
